...
def create_plot_gif(title: str,
    ...
    if load_saved == False:
        for i in tqdm(range(len(models)), 'Creating GIF'):
            save_decision_regions(i, models[i], X, Y, params, title)
...
# decision regions plot saving
def save_decision_regions(index: int,
                          model, 
                          X: np.ndarray,
                          Y: np.ndarray,
    ...
    file_name = file_title + '_{:03}.png'.format(index)
...
def save_hlines(index: int, 
    ...
    font_s = 12
    plt.title(title + ' {:03}'.format(index) + '\n', fontsize=font_s+4)
    ...
    plt.fill_between(x, value, where=value < 0, color='red', alpha=0.5)
    plt.fill_between(x, value, where=value > 0, color='green', alpha=0.5)
